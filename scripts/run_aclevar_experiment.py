#!/usr/bin/env python3
"""Run ACLE-VAR experiment on ETF returns.
...
ACLE-VAR applies the same significance-testing methodology as ORACLE-VARX but
...
Unlike ACLE-VARX, this version does NOT include VIX in the state vector.
...
Example:
    Run with verbose output:
        ...
        python3 scripts/run_aclevar_experiment.py --verbose

    Run with smaller dataset for quick testing:

        python3 scripts/run_aclevar_experiment.py --n-days 600 --verbose
"""
...
def main(
    ...
    device: str = "cpu",
    output_dir: str = "results/aclevar",
    experiment_name: str = None,
    show_plots: bool = True,
    verbose: bool = False,
):
    """Run ACLE-VAR experiment.
    ...
    if experiment_name is None:
        ...
        experiment_name = f"aclevar_{timestamp}"
    ...
    # Load ETF returns and SPY for benchmark (no VIX)
    tickers = ETFS + ["SPY"]

    Y, _, dates, loaded_tickers = prepare_tensors(
        tickers=tickers,
        confounder_names=None,  # No confounders for ACLE-VAR
        n_days=n_days,
        device=device,
    )
    ...
    print(f"  Model assets: {etf_tickers}")
    print(f"  Shape: {Y_etf.shape}")

    # =========================================================================
    # Step 2: Run ACLE-VAR Model
    # =========================================================================
    print("\n" + "-" * 40)
    print("Step 2: Running ACLE-VAR Model")
    print("-" * 40)
    ...
    plot_strategy_comparison(
        ...
        title=f"ACLE-VAR: Market-Adjusted Strategy Comparison\n{result.dates[0]} to {result.dates[-1]}",
    ...
    plot_strategy_comparison(
        ...
        title=f"ACLE-VAR: Strategy Comparison vs SPY\n{result.dates[0]} to {result.dates[-1]}",
    ...
    for label, result_day_idx in targets:
        ...
        per_p_coefs = refit_var_coefficients_for_day(
            Y=Y_etf, day_idx=abs_day_idx, p_star=p_star,
        ...
        plot_coefficient_evolution_per_p(
            per_p_coefs,
            title=f"ACLE-VAR: Coefficient Evolution p*={p_star} ({date_str})",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run ACLE-VAR experiment on ETF returns")
    ...
    parser.add_argument("--output-dir", type=str, default="results/aclevar", help="Output directory")
