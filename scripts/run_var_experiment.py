#!/usr/bin/env python3
"""Run Plain VAR experiment on ETF returns.

This script:
1. Loads ETF returns from OPCL file
2. Runs fit_var() with specified parameters
...
def main(
    ...
    if experiment_name is None:
        ...
        experiment_name = f"var_{timestamp}"
    ...
    # Load ETF returns and SPY for benchmark
    tickers = ETFS + ["SPY"]  # Add SPY for benchmark

    Y, _, dates, loaded_tickers = prepare_tensors(
        tickers=tickers,
        confounder_names=None,  # No confounders for plain VAR
        n_days=n_days,
        device=device,
    )
    ...
    # =========================================================================
    # Step 2: Run VAR Model
    # =========================================================================
    print("\n" + "-" * 40)
    print("Step 2: Running VAR Model")
    print("-" * 40)
    ...
    plot_strategy_comparison(
        ...
        title=f"VAR: Market-Adjusted Strategy Comparison\n{result.dates[0]} to {result.dates[-1]}",
    ...
    plot_strategy_comparison(
        ...
        title=f"VAR: Strategy Comparison vs SPY\n{result.dates[0]} to {result.dates[-1]}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run VAR experiment on ETF returns")
    ...
    parser.add_argument("--output-dir", type=str, default="results/var", help="Output directory")
